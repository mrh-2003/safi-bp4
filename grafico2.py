...
def render(df):
    ...
    for moneda, emoji in [("SOLES", "🇵🇪"), ("DOLARES", "🇺🇸")]:
        ...
        with st.expander(f"🔧 Filtros — {moneda}", expanded=True):
            ...
            sel_gen = st.multiselect("Genérico", genericos, default=genericos, key=f"g2_{moneda}_gen")
            dfm = dfm[dfm["GENÉRICO"].isin(sel_gen)]
            # Filtro Descripción
            descripciones = sorted(dfm["DESCRIPCION"].dropna().unique())
            sel_desc = st.multiselect("Descripción (opcional)", descripciones, default=[], key=f"g2_{moneda}_desc")
            if sel_desc:
        ...
        count = len(dfm)
        c1.metric("💰 Total Neto", f"{total:,.2f}")
        c2.metric("📥 Ingresos", f"{ing:,.2f}")
        c3.metric("📤 Egresos", f"{egr:,.2f}")
        c4.metric("📝 Movimientos", f"{count:,}")

        # ── Sumatoria por Genérico ────────────────────────────────────
        st.markdown("#### Por Genérico")
        gen_pivot = dfm.pivot_table(
            values="CARGO_ABONO", index="GENÉRICO", columns="TIPO",
            aggfunc="sum", fill_value=0, margins=True, margins_name="TOTAL"
        ).reset_index()
        st.dataframe(gen_pivot.style.format({c: "{:,.2f}" for c in gen_pivot.columns if c != "GENÉRICO"}),
                      use_container_width=True)

        # Gráfico barras por genérico
        gen_data = dfm.groupby(["GENÉRICO", "TIPO"])["CARGO_ABONO"].sum().reset_index()
        if not gen_data.empty:
            fig_bar = px.bar(
                gen_data, x="GENÉRICO", y="CARGO_ABONO", color="TIPO",
                barmode="group", template="plotly_dark",
                color_discrete_map={"Ingreso": COLORS["ingreso"], "Egreso": COLORS["egreso"]},
                title=f"Cargo/Abono por Genérico — {moneda}",
            )
            fig_bar.update_layout(xaxis_tickangle=-45, height=450)
            st.plotly_chart(fig_bar, use_container_width=True, key=f"bar_g2_{moneda}")

        # ── Sumatoria por Fecha (mensual) ─────────────────────────────
        st.markdown("#### Por Mes")
        dfm["AnoMes"] = dfm["FECHA_OPER_"].dt.to_period("M").astype(str)
        mes_pivot = dfm.pivot_table(
            values="CARGO_ABONO", index="AnoMes", columns="TIPO",
            aggfunc="sum", fill_value=0, margins=True, margins_name="TOTAL"
        ).reset_index()
        st.dataframe(mes_pivot.style.format({c: "{:,.2f}" for c in mes_pivot.columns if c != "AnoMes"}),
                      use_container_width=True)

        # Gráfico líneas mensual
        mes_data = dfm.groupby(["AnoMes", "TIPO"])["CARGO_ABONO"].sum().reset_index()
        if not mes_data.empty:
            fig_line = px.line(
                mes_data, x="AnoMes", y="CARGO_ABONO", color="TIPO",
                markers=True, template="plotly_dark",
                color_discrete_map={"Ingreso": COLORS["ingreso"], "Egreso": COLORS["egreso"]},
                title=f"Evolución Mensual — {moneda}",
            )
            fig_line.update_layout(height=400)
            st.plotly_chart(fig_line, use_container_width=True, key=f"line_g2_{moneda}")

        # ── Tabla detallada ───────────────────────────────────────────
        with st.expander(f"📋 Detalle completo — {moneda}"):
            show = dfm[["FECHA_OPER_", "DESCRIPCION", "GENÉRICO", "TIPO", "CARGO_ABONO", "SALDO_CONTABLE"]].copy()
