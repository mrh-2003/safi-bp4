...
def render(df):
    ...
    for moneda, emoji in [("SOLES", "🇵🇪"), ("DOLARES", "🇺🇸")]:
        ...
        ing_group = ingresos.groupby(ingresos["FECHA_OPER_"].dt.date)["SALDO_CONTABLE"].last().sort_index()
        egr_group = egresos.groupby(egresos["FECHA_OPER_"].dt.date)["SALDO_CONTABLE"].last().sort_index()

        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=list(ing_group.index), y=list(ing_group.values),
        ...
        fig.add_trace(go.Scatter(
            x=list(egr_group.index), y=list(egr_group.values),
        ...
        with st.expander(f"📋 Tabla de datos — {moneda}"):
            tabla = dfm[["FECHA_OPER_", "TIPO", "DESCRIPCION", "GENÉRICO", "CARGO_ABONO", "SALDO_CONTABLE"]].copy()
            tabla["FECHA_OPER_"] = tabla["FECHA_OPER_"].dt.strftime("%d/%m/%Y")
            st.dataframe(tabla, use_container_width=True, height=400)

        # KPIs
        c1, c2, c3 = st.columns(3)
        c1.metric("Total Ingresos", f"{ingresos['CARGO_ABONO'].sum():,.2f}")
        c2.metric("Total Egresos", f"{egresos['CARGO_ABONO'].sum():,.2f}")
        c3.metric("Neto", f"{dfm['CARGO_ABONO'].sum():,.2f}")
        st.divider()
